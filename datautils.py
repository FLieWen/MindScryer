...
# Ablation parameters for prompt source switching
parser.add_argument('--pos_prompt_type', type=str, default='blip2',
                    choices=['blip2', 'imagenet_label'],
                    help='D2 ablation: source of positive text prompts (blip2=BLIP-2 captions, imagenet_label=ImageNet class names)')
parser.add_argument('--neg_prompt_type', type=str, default='mistral',
                    choices=['mistral', 'random', 'none'],
                    help='D3 ablation: source of negative text prompts (mistral=Mistral-7B, random=random class, none=zeros)')
...
def load_EEG(Path='data/EEG/'):
    # clip_path='data/image_text_CLIP/'
    clip_path='data/image_CLIP/'

    # D2 ablation: switch positive prompt source
    if opt.pos_prompt_type == 'imagenet_label':
        clip_moreinf_path = 'data/imagelabel_text_CLIP/'
    else:
        clip_moreinf_path = 'data/image_text_CLIP/'  # baseline: BLIP-2 captions

    neg_img_path = 'data/negative_image_CLIP/'

    # D3 ablation: switch negative text prompt source
    neg_prompt_type = opt.neg_prompt_type
    if neg_prompt_type == 'random':
        neg_text_path = 'data/imagelabel_text_CLIP/'  # use random ImageNet labels as negatives
        import random as _random
    elif neg_prompt_type == 'none':
        neg_text_path = None  # signal to use zero vectors
    else:
        neg_text_path = 'data/negative_text_CLIP/'  # baseline: Mistral-7B negatives
    ...
    train_neg_img_list, train_neg_text_list = [], []  # 负样本
    ...
    for i in range(0, len(train_dataset)):
        ...
        # 调试输出，查看CLIP向量的形状
        # print(f"Shape of target_vec (CLIP向量) for {img_name}: {target_vec.shape}")
        ...
        # 加载负样本
        neg_img_file = neg_img_path + img_name + '_sample1.csv'
        neg_img_vec = torch.tensor(pd.read_csv(neg_img_file, header=None).values).reshape(1, -1).squeeze()

        # D3 ablation: negative text loading
        if neg_prompt_type == 'none':
            neg_text_vec = torch.zeros_like(target_moreinf_vec)
        elif neg_prompt_type == 'random':
            random_files = os.listdir(neg_text_path)
            random_file = _random.choice(random_files)
            neg_text_vec = torch.tensor(pd.read_csv(neg_text_path + random_file, header=None).values).reshape(1, -1).squeeze()
        else:
            neg_text_file = neg_text_path + img_name + '.csv'
            neg_text_vec = torch.tensor(pd.read_csv(neg_text_file, header=None).values).reshape(1, -1).squeeze()

        train_neg_img_list.append(neg_img_vec)
        train_neg_text_list.append(neg_text_vec)
    ...
    TRAIN_NEG_IMG = torch.stack(train_neg_img_list, dim=0)
    # print(f"Shape of TRAIN_NEG_IMG: {TRAIN_NEG_IMG.shape}")
    TRAIN_NEG_TEXT = torch.stack(train_neg_text_list, dim=0)
    ...
    val_neg_img_list, val_neg_text_list = [], []
    ...
    for i in range(0, len(val_dataset)):
        ...
        # D3 ablation: negative text loading (val)
        if neg_prompt_type == 'none':
            neg_text_vec = torch.zeros_like(target_moreinf_vec)
        elif neg_prompt_type == 'random':
            random_files = os.listdir(neg_text_path)
        ...
        val_neg_img_list.append(neg_img_vec)
        val_neg_text_list.append(neg_text_vec)
    ...
    VAL_NEG_IMG = torch.stack(val_neg_img_list, dim=0)
    VAL_NEG_TEXT = torch.stack(val_neg_text_list, dim=0)
    ...
    TEST_NEG_IMG = torch.stack(val_neg_img_list, dim=0)
    TEST_NEG_TEXT = torch.stack(val_neg_text_list, dim=0)
    ...
    # print(f"Shape of ALL_TRAIN_CLIP: {ALL_TRAIN_CLIP.shape}")
    ALL_TRAIN_MOREINF_CLIP=torch.cat([TRAIN_MOREINF_CLIP, VAL_MOREINF_CLIP])
    ALL_TRAIN_NEG_IMG = torch.cat([TRAIN_NEG_IMG, VAL_NEG_IMG])
    # print(f"Shape of ALL_TRAIN_NEG_IMG: {ALL_TRAIN_NEG_IMG.shape}")
    ALL_TRAIN_NEG_TEXT = torch.cat([TRAIN_NEG_TEXT, VAL_NEG_TEXT])

    print('data loaded')

    return [np.array(ALL_TRAIN_DATA), np.array(ALL_TRAIN_LABEL),np.array(ALL_TRAIN_CLIP),np.array(ALL_TRAIN_MOREINF_CLIP),np.array(ALL_TRAIN_NEG_IMG), np.array(ALL_TRAIN_NEG_TEXT)], \
           [np.array(TRAIN_DATA), np.array(TRAIN_LABEL),np.array(TRAIN_CLIP),np.array(TRAIN_MOREINF_CLIP),np.array(TRAIN_NEG_IMG), np.array(TRAIN_NEG_TEXT)], \
           [np.array(TEST_DATA), np.array(TEST_LABEL),np.array(TEST_CLIP),np.array(TEST_MOREINF_CLIP), np.array(TEST_NEG_IMG), np.array(TEST_NEG_TEXT)]
